...
var esbuild_plugin_lit_css_exports = {};
__export(esbuild_plugin_lit_css_exports, {
  litCssPlugin: () => litCssPlugin
});
...
function litCssPlugin(options) {
  const _a = options != null ? options : {}, { filter = /\.css$/, specifier, tag, uglify } = _a, rest = __objRest(_a, ["filter", "specifier", "tag", "uglify"]);
  return {
    name: "lit-css",
    setup(build) {
      const loader = "js";
      build.onLoad({ filter }, (args) => __async(this, null, function* () {
        const css = yield (0, import_promises.readFile)(args.path, "utf8");
        const filePath = args.path;
        try {
          const contents = yield transform(__spreadValues({ css, specifier, tag, uglify, filePath }, rest));
...
module.exports = __toCommonJS(esbuild_plugin_lit_css_exports);
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  litCssPlugin
});
//# sourceMappingURL=esbuild-plugin-lit-css.cjs.map
