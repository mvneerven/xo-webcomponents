...
class PropertyMapper {
        ...
        for (let prop in properties) {
            ...
            if (nested) {
                if (PropertyMapper.elementSupportsProperty(nested, prop)) {
                    nested[prop] = value !== null && value !== void 0 ? value : "";
                }
                else {
    ...
    static match(s, callback) {
        ...
        return s.replace(/(#\/[A-Za-z_]+[A-Za-z_0-9\/@]*[A-Za-z_]+[A-Za-z_0-9]*)(?=[\s+\/*,.?!;'")]|$)/gm, (match, token, r) => {
    ...
    replaceVar(binding, prop, value) {
        ...
        let varRes, result = PropertyMapper.match(binding.rawValue, (variable, origString) => {
    ...
    tryAutoComplete(input, textInput, autoComplete) {
        if (autoComplete && autoComplete.items) {
            let ac = new AutoComplete_1.default(input, textInput, autoComplete);
            ac.attach(input);
        }
    }
